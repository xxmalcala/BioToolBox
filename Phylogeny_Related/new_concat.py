...
def parse_tree(msa_file, tree_file):
    try:
        ...
        try:
            t = Tree(tree_file.replace(".fas",".tre"))
    ...
    if mjr_c:
        if len(set(mjr_c)) == 1:
            ...
            if recursion_depth < 1:
                with open(f'CladeSizes/{out_name}_CladeSize.txt','a') as w:
...
def eval_paralogs(out_name, t, msa_file):
    ...
    plogs = []
    for line in open(f'CladeSizes/{out_name}_CladeSize.txt').readlines():
...
def get_files(msa_dir, tree_dir):
    ...
    file_pair = [(f'{msa_dir}/{f}',f'{tree_dir}/{f.replace("fas","ReRooted.tre")}') for f in os.listdir(msa_dir) if f.endswith('.fas')]
...
if __name__ == '__main__':
    if len(sys.argv[1:]) != 4:
        ...
        if len(sys.argv[1:]) == 4:
            ...
            project = sys.argv[4]
    Path('Pruned').mkdir(parents = True, exist_ok = True)
    Path('CladeSizes').mkdir(parents = True, exist_ok = True)
